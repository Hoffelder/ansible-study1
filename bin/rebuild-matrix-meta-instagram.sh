...
if [$# -eq 0 ]; then
    echo "Error: No argument supplied. Please provide the provide the path to the roles/custom directory."
    exit 1
fi 
...
instagram-role_path=$roles_path/matrix-bridge-mautrix-meta-instagram
...
sed --in-place 's/matrix_mautrix_meta_instagram_meta_mode: \(.*\)/matrix_meta_instagram_meta_mode: instagram/g' $instagram_role_path/defaults/main.yaml
sed --in-place 's/matrix_mautrix_meta_instagram_identifier:\(.*\)/matrix_mautrix_meta_instagram_identifier: matrix-mautrix-meta-instagram/g' $instagram_role_path/defaults/main.yaml
